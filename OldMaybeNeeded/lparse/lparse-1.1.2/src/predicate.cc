...
int is_negation_name(const char *st)
{
  while (*st == '_')
    st++;
...
const char *complement_name(const char *nm)
{
  int len = strlen(nm);
...
const char *negation_name(const char *nm)
{
  static string str;
  //  int len = strlen(nm);
  int underscores = 0;
...
long Predicate::DefinePredicate(const char *nm, int ar, long ln)
{
  long p = -1;
...
long Predicate::DefineComplement(long p)
{
  ...
  const char *new_name = complement_name(original->Name());
...
Instance Predicate::GetArgumentInstanceFromString(const char *st, int n)
{
  Instance result = -1;
...
void Predicate::CheckImplicitDomains()
{
  ...
  long p = -1;
  bool first = 1;
  const char *error_template1 =
    "%s%sthe predicate `%s' in the domain statement %s. There is "
    "a cycle: ";
  const char *error_template2 = " in the dependency graph. (Did you use "
...
void Predicate::DefineSignature(long *)
{
  //   if (signature) {
