...
state_helper(burdened).
burdened :- rock(R), holds(bagged(R)).
...
possible(mine(R)) :- holds(present(R)), holds(position(P1)), link(P2,P1), in(R,P2), \+ burdened.
possible(mine(R)) :- holds(present(R)), holds(position(P1)), link(P1,P2), in(R,P2), \+ burdened.

possible(up_to(P2)) :- holds(position(P1)), link(P2,P1), \+ occupied(P2).
...
ui_title('DrillBot 5999').
ui_instruction('DrillBot 5999 (aka "You Have to Mine the Ore") \n\
\n\
INSTRUCTIONS:\n\
- you control mining robot "db6k"\n\
- left-click ROCKS to mine them [one rock at a time]\n\
- left-click empty CAVERNS to enter them \n\
- left-click the BASE to trade when docked\n\
...
ui_details(bagged(R)) :- holds(bagged(R)).
ui_details('Your bags are full!') :- burdened.
