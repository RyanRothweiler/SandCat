...
public:
  ...
  void SetValue(const char *key, long val, int arity = -1);

  // check if there is an item 'key' with different arity than 'arity' 
  long CheckName(const char *key, int arity);
...
public:
  ...
  long Insert(const char *key);
  long Register(const char *key, InstFunc p, int val = 0); 
  long Lookup(const char *key);
  long Define(const char *symbol, char *value, int val = 0);

  InstFunc GetFunction(const char *key);
  long Size() { return size; }
